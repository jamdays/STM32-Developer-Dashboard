...
int sensor_reading(const char *sensor_name, char *buf, size_t buf_len)
{
    ...
    if (strcmp(sensor_name, "hts221") == 0) {
        ...
        used = snprintf(buf, buf_len, "{\"temperature\": %.3f, \"humidity\": %.3f}",
        ...
        used = snprintf(buf, buf_len,
                        "{\"pressure\": %d}",
                        sensor_value_to_float(&val[0])); // Convert to Pa
    }
        ...
        used = snprintf(buf, buf_len, "{\"mag\": [%.3f, %.3f, %.3f]}", val[0].val2, val[1].val2, val[2].val2);
    }
